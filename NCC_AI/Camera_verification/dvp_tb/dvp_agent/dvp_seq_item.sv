...
class dvp_seq_item extends uvm_sequence_item; 
  ...
  // 3. Timing Metadata 
  bit      is_start_of_frame;  // Flag for VSYNC
  rand int h_blank_cycles;     // Delay between normal lines
  rand int v_pulse_cycles;     // How long VSYNC stays HIGH
  rand int v_blank_cycles;     // Delay before first line starts (Back Porch)
  ...
  // Array strictly sized to exactly 1 line
  ...
  // Vertical Timing (Only applies to Line 0)
  constraint c_v_timing {
    if (is_start_of_frame == 1) {
      v_pulse_cycles inside {[3:10]};    // Realistic VSYNC pulse width
      v_blank_cycles inside {[100:500]}; // Realistic Vertical Blanking time
    } else {
      v_pulse_cycles == 0;
      v_blank_cycles == 0;
    }
